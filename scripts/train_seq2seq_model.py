...
from models.temporal_models.seq2seq import Seq2Seq
...
def split_latents(dataset, input_size, output_size=0):

    if output_size == 0:
        output_size = dataset.shape[1] - input_size

    x = dataset[:,:input_size,:]
    y = dataset[:,input_size:(input_size+output_size), :]
...
if __name__ == "__main__":
    ...
    X_train, y_train = split_latents(train_latents, input_size=math.ceil(train_latents.shape[1]/2))
    val_X_train, val_y_train = split_latents(val_latents, input_size=math.ceil(val_latents.shape[1]/2))
    ...
    l_model = Seq2Seq(input_dim=LATENT_SIZE, output_dim=LATENT_SIZE, hidden_dim=HIDDEN_SIZE, n_rnn_layers=NO_RNN_LAYERS, device=DEVICE)
    ...
    if TRAIN:
        ...
        for epoch in range(EPOCHS):
            ...
            for X_batch, y_batch in loader:
                y_pred = l_model(X_batch, y_batch.shape[1])
            ...
            with torch.no_grad():
                for val_X_batch, val_y_batch in val_loader:
                    y_pred = l_model(val_X_batch, val_y_batch.shape[1])
            ...
            if RECONSTRUCT_AUDIO:
                for val_X_batch, val_y_batch in val_loader:
                    original_latent = torch.cat((val_X_batch, val_y_batch), dim=1)
                    y_pred = l_model(val_X_batch, val_y_batch.shape[1])
                    gen_latent = torch.cat((val_X_batch, y_pred), dim=1)
                    orig_audio = w_model.decode(original_latent, noise_synth=NOISE_SYNTH)["audio"]
                    gen_audio = w_model.decode(gen_latent, noise_synth=NOISE_SYNTH)["audio"]
                    torchaudio.save(f'./original.wav', orig_audio.cpu(), SAMPLE_RATE)
                    torchaudio.save(f"./gen.wav", gen_audio.cpu(), SAMPLE_RATE)



            print("Epoch %d: train RMSE %.10f, validation RMSE %.10f" % (epoch, train_rmse, val_rmse))
